...
TOKEN_IF = re.compile(r'\bif\s*\(')
TOKEN_ELSE_IF = re.compile(r'\belse\s+if\s*\(')
TOKEN_ELSE = re.compile(r'\belse\s*\{')
TOKEN_DEPS_ASSIGN = re.compile(r'\b(deps|public_deps|external_deps|public_external_deps)\s*\+?=\s*')
TOKEN_VAR_ASSIGN = re.compile(r'([A-Za-z_][A-Za-z0-9_]*)\s*=\s*')
...
def resolve_label_candidates(
    ...
    # Optimization: if all variables in label are locally defined and we resolved it (base is clean),
    # assume we don't need to guess.
    if local_vars and candidates and all(v in local_vars for v in vars_in_label):
        return candidates
...
def parse_target_deps(
    ...
    # scope_vars starts as copy of file variables
    scope_vars = variables.copy()
    ...
    while i < length:
        ...
        # 1. else if
        m = TOKEN_ELSE_IF.match(body, i)
        if m:
            start_cond = m.end() - 1
            end_cond = find_closing_paren(body, start_cond)
            if end_cond != -1:
                cond_expr = body[start_cond+1:end_cond].strip()
                j = end_cond + 1
                while j < length and body[j].isspace(): j += 1
                if j < length and body[j] == '{':
                    stack.append(('elif', cond_expr, list(pending_false_conds)))
                    pending_false_conds.append(cond_expr)
                    i = j + 1
                    continue

        # 2. if
        m = TOKEN_IF.match(body, i)
        if m:
            start_cond = m.end() - 1
            end_cond = find_closing_paren(body, start_cond)
            if end_cond != -1:
                ...
                while j < length and body[j].isspace(): j += 1
                if j < length and body[j] == '{':
                    stack.append(('if_true', cond_expr, []))
                    pending_false_conds = [cond_expr]
                    i = j + 1
                    continue

        # 3. else
        m = TOKEN_ELSE.match(body, i)
        if m:
        ...
        # 6. deps assignment
        m = TOKEN_DEPS_ASSIGN.match(body, i)
        if m:
            dep_type = m.group(1)
            j = m.end()
            while j < length and body[j].isspace(): j += 1
            ...
            if j < length and body[j] == '[':
                ...
                while j < length and (body[j].isalnum() or body[j] == '_' or body[j] == '$' or body[j] == ':' or body[j] == '/' or body[j] == '"' or body[j] == '.'):
                    j += 1
                value_end = j

            dep_block = body[value_start:value_end]
            i = j

            cond = current_condition(stack)
            if not dep_block.startswith('['):
                var_name = dep_block.strip()
                resolved_val = scope_vars.get(var_name)
                if resolved_val:
                    dep_block = resolved_val
                else:
                     warn(f"[WARN] deps variable assignment not resolved: {var_name}")

            for label_match in LABEL_RE.finditer(dep_block):
                if dep_type in ('external_deps', 'public_external_deps'):
                    ...
                    resolved_candidates = resolve_label_candidates(
                        ...
                        scan_root,
                        current_file,
                        local_vars=set(scope_vars.keys()),
                    )
        ...
        m_var = TOKEN_VAR_ASSIGN.match(body, i)
        if m_var:
            var_name = m_var.group(1)
            if var_name not in ('deps', 'public_deps', 'external_deps', 'public_external_deps'):
                j = m_var.end()
                while j < length and body[j].isspace(): j += 1
                if j < length and body[j] == '"':
                    start_quote = j
